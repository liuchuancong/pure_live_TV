...
onMounted(async () => {
  const currentCookie = await api.getCookie('douyu')
...
function handleCookieInput() {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (val) api.updateCookie('douyu', val)
  }, 400)
}
...
async function submitCookie() {
  ...
  if (!val) return
  loading.value = true
  const res = await api.updateCookie('douyu', val)
  toast.show(res.isOk ? '斗鱼Cookie已同步至电视端' : res.msg || '同步失败，请重试', res.isOk ? 'success' : 'error')
  loading.value = false
}
</script>
```
