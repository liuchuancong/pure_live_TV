...
const loadSavedCookie = async () => {
  const currentCookie = await api.getCookie('cc')
  cookieData.value = currentCookie
}

onMounted(loadSavedCookie)
...
watch(cookieData, () => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (val) api.updateCookie('cc', val)
  }, 400)
})
...
async function submitCookie() {
  const val = cookieData.value.trim()
  if (!val) {
    toast.show('请输入有效的网易CC Cookie', 'error')
    return
  }
  loading.value = true
  const res = await api.updateCookie('cc', val)
  toast.show(res.isOk ? '网易CC Cookie已同步至电视端' : res.msg || '同步失败，请重试', res.isOk ? 'success' : 'error')
  loading.value = false
}
</script>
