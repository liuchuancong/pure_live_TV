...
onMounted(async () => {
  const currentCookie = await api.getCookie('huya')
...
function handleCookieInput() {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (val) api.updateCookie('huya', val)
  }, 400)
}
...
async function submitCookie() {
  ...
  if (!val) return
  loading.value = true
  const res = await api.updateCookie('huya', val)
  toast.show(res.isOk ? '虎牙Cookie已同步至电视端' : res.msg || '同步失败，请重试', res.isOk ? 'success' : 'error')
  loading.value = false
}
</script>
