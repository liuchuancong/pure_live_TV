  ...
  <div class="p-3 sm:p-4 space-y-4 sm:space-y-5 text-left">
    <div class="p-3 sm:p-4 bg-ios-card dark:bg-ios-card border border-ios-border/20 dark:border-ios-border/30 rounded-2xl shadow-[0_8px_30px_var(--color-ios-shadow)] space-y-3 sm:space-y-4">
      <div class="flex items-center justify-between px-0.5">
        ...
        <button v-if="cookieData.trim()" class="text-xs font-semibold text-ios-blue cursor-pointer active:opacity-60 transition-opacity" @click="clearCookie">一键清空</button>
      </div>
      <textarea
        v-model="cookieData"
        :placeholder="`在此输入 ${siteName} Cookie...`"
    ...
    <div class="flex gap-3">
      ...
      <button
        class="flex-1 py-3 sm:py-3.5 bg-zinc-500/10 text-red-500 font-bold rounded-2xl text-sm sm:text-[15px] transition-all md:hover:scale-[1.01] active:scale-[0.98] cursor-pointer"
        @click="submitClear"
      >
        清除电视上的 Cookie
      </button>
...
onMounted(async () => {
  cookieData.value = await api.getCookie(props.site)
})
...
async function submitCookie() {
  ...
  if (!val) return
  loading.value = true
  const res = await api.updateCookie(props.site, val)
  toast.show(res.isOk ? `${props.siteName} Cookie已同步至电视端` : res.msg || '同步失败，请重试', res.isOk ? 'success' : 'error')
  loading.value = false
}

async function submitClear() {
  if (!window.confirm(`清除电视上的 ${props.siteName} Cookie？`)) return
  cookieData.value = ''
  const res = await api.updateCookie(props.site, '')
