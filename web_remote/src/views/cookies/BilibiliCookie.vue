<template>
  <div class="p-3 sm:p-4 space-y-4 sm:space-y-5 text-left">
    <div class="p-3 sm:p-4 bg-ios-card dark:bg-ios-card border border-ios-border/20 dark:border-ios-border/30 rounded-2xl shadow-[0_8px_30px_var(--color-ios-shadow)] space-y-3 sm:space-y-4">
      ...
      <textarea
        v-model="cookieData"
        placeholder="在此输入 Bilibili Cookie..."
      ...
      <div class="flex items-center justify-between px-0.5 text-[10px] sm:text-[11px] text-ios-gray dark:text-ios-gray font-medium">
        <div class="flex items-center gap-2">
          ...
          <button class="relative w-9 h-5 rounded-full transition-colors duration-200" :class="autoSync ? 'bg-ios-blue' : 'bg-ios-border/40 dark:bg-ios-border/60'" @click="autoSync = !autoSync">
            <span class="absolute top-0.5 left-0.5 w-4 h-4 rounded-full shadow transition-transform duration-200 dark:bg-white" :class="autoSync ? 'translate-x-5 bg-white' : 'translate-x-0.5 bg-ios-card dark:bg-ios-bg'"></span>
          </button>
        </div>
...
onMounted(async () => {
  const currentCookie = await api.getCookie('bilibili')
...
watch(cookieData, () => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (val) api.updateCookie('bilibili', val)
  }, 400)
})
...
async function submitCookie() {
  const val = cookieData.value.trim()
  if (!val) {
    toast.show('请填写Cookie内容', 'error')
    return
  }
  loading.value = true
  const res = await api.updateCookie('bilibili', val)
  toast.show(res.isOk ? '已同步至电视端' : res.msg || '同步失败', res.isOk ? 'success' : 'error')
  loading.value = false
}
</script>
