...
class FavoriteRoomController extends _$FavoriteRoomController {
  ...
  // ------------------------------------------------------------------
  // build-time normalization (ported from pure_live onInit)
  // ------------------------------------------------------------------
  ...
  /// 同步自 pure_live：解析收藏分区（含校验与归一化），不触发持久化。
  ...
  /// 同步自 pure_live：从备份根配置中提取 favorite 分区。
